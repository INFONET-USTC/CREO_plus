...
def encode_action(
    ...
    previous_rate_bps = max(
        int(state["pacing_rate_bps"]),
        int(state["throughput_bps"]),
        1,
    )
    target_rate_bps = max(
        1,
        round(previous_rate_bps * float(response["action_multiplier"])),
    )
...
def main() -> int:
        ...
        with log_path.open("a", encoding="utf-8") as action_log:
            while running:
                ...
                if args.max_messages:
                    limit = min(limit, args.max_messages - states_received)
                states = []
                while len(states) < limit:
                    ...
                    if not payload:
                        break
                    states.append(decode_state(payload))
                if not states:
                    continue

                observed_batch_sizes.append(len(states))
                responses = service.infer([model_request(state) for state in states])
                for state, response in zip(states, responses):
                    ...
                    row = {
                        "time_unix": time.time(),
                        "inference_batch_size": len(states),
    ...
    summary = {
        **metadata,
        ...
        "max_inference_batch_size": max(observed_batch_sizes, default=0),
