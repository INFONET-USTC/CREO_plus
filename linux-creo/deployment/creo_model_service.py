...
class PathContext:
    ...
    def build(self, telemetry: dict[str, float]) -> dict[str, np.ndarray]:
        ...
        metric = np.clip(
            metric,
            np.asarray([0, 0, 0, 0, 0, -10, 0, 0, 0.1], dtype=np.float32),
            np.asarray([1, 2, 2, 10, 10, 10, 10, 4, 3], dtype=np.float32),
        )
        ...
        return {
            "capacity": np.clip(self.pad(denoised, self.capacity_window) / scale, 0, 2),
            "trend": np.clip(self.pad(trend.astype(np.float32), self.capacity_window) / scale, 0, 2),
            "fluct": np.clip(
                self.pad((denoised - trend).astype(np.float32), self.capacity_window) / scale,
                -2,
                2,
            ),
...
def default_checkpoint() -> Path:
    return (
        ...
        / "LEOSN Network Simulation/ns3-creo/ns3-overlay/contrib/ai/examples/rl-tcp/use-gym/models/creo_single.pt"
    )
