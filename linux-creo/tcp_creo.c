...
MODULE_PARM_DESC(update_interval_us, "DeltaT0 upper bound for the RTT-adaptive CREO statistical period");
...
static u32 creo_statistical_period_us(struct sock *sk,
				      const struct rate_sample *rs)
{
	const struct tcp_sock *tp = tcp_sk(sk);
	const struct creo *ca = inet_csk_ca(sk);
	u32 previous_rtt_us = ca->last_rtt_us;
	u32 srtt_us = tp->srtt_us ? tp->srtt_us >> 3 : 0;
	u32 period_us = max_t(u32, update_interval_us, 1U);

	if (!previous_rtt_us && rs && rs->rtt_us > 0)
		previous_rtt_us = rs->rtt_us;
	if (previous_rtt_us)
		period_us = min(period_us, max_t(u32, previous_rtt_us / 2U, 1U));
	if (srtt_us)
		period_us = min(period_us, srtt_us);

	return max_t(u32, period_us, 1000U);
}
...
static void creo_update(struct sock *sk, const struct rate_sample *rs)
{
	...
	period_us = creo_statistical_period_us(sk, rs);
	if (ca->last_update_us &&
	    now_us - ca->last_update_us < period_us)
		return;
