...
std::vector<Ptr<PacketSink>> g_dataSinks;
...
{
    uint64_t rxBytes = 0;
    for (const auto& sink : g_dataSinks)
    {
              ...
              double processDelayMs)
{
    ...
    double rawDelay = TraceOrSynthetic(latencyTrace, line, 1, now, 2.2, 0.4);
    if (rawDelay > 0.0 && rawDelay < 0.1)
    {
        rawDelay *= 1000.0;
    }
    double delayMs = std::max(1.0, rawDelay);
    ...
    SetCreoEvaluationCapacity(bwMbps);
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("adaptiveSp", "Use the RTT-adaptive statistical period", adaptiveSp);
    ...
    for (uint32_t i = 0; i < flows; ++i)
    {
        ...
        g_dataSinks.push_back(DynamicCast<PacketSink>(sinkApp.Get(0)));
    ...
    Simulator::Schedule(Seconds(0.101), &TraceThroughput);
