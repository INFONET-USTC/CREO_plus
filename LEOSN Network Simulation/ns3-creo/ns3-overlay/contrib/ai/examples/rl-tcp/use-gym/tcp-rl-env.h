...
/** Set the trace capacity exposed only in the simulation evaluation field. */
void SetCreoEvaluationCapacity(double capacityMbps);
...
class TcpEnvBase : public OpenGymEnv
{
  ...
  protected:
    ...
    // actions
    uint32_t m_new_ssThresh{0};
    uint32_t m_new_cWnd{0};
    uint32_t m_new_pacing_rate{0};
};
...
class TcpTimeStepEnv : public TcpEnvBase
{
  ...
  private:
    ...
    Time m_timeStep;
    Time m_lastObservationTime{MicroSeconds(0)};
    ...
    Ptr<TcpSocketState> m_controlTcb;
    uint32_t m_burstTxCount{0};
    bool m_burstGapPending{false};
    bool m_capacityWindowActive{false};
    uint64_t m_totalAckedBytes{0};
    uint64_t m_capacityWindowStartBytes{0};
    Time m_miniWindowStart{MicroSeconds(0.0)};
    Time m_lastCapacityAckTime{MicroSeconds(0.0)};
