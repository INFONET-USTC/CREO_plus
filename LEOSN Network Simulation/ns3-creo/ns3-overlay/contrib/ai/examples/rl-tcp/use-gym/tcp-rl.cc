...
bool
TcpRlBase::HasCongControl() const
{
    return true;
}

void
TcpRlBase::CongControl(Ptr<TcpSocketState> tcb,
                       const TcpRateOps::TcpRateConnection& rc,
                       const TcpRateOps::TcpRateSample& rs)
{
    // CREO+ applies cwnd and pacing actions in TcpTimeStepEnv::IncreaseWindow.
    // Advertising CongControl prevents TcpSocketBase from overwriting that
    // pacing rate with its generic cwnd/RTT calculation after each ACK.
    (void)tcb;
    (void)rc;
    (void)rs;
}
