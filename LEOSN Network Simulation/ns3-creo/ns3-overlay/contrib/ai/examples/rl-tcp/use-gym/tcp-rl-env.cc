...
namespace
{
uint64_t g_creoEvaluationCapacityBps{1};
}

void
SetCreoEvaluationCapacity(double capacityMbps)
{
    g_creoEvaluationCapacityBps = static_cast<uint64_t>(
        std::max(0.0, capacityMbps) * 1000000.0);
}
...
{
    // [0] target cwnd in bytes
    // [1] target sending rate in bit/s
...
TcpTimeStepEnv::GetObservationSpace()
{
    ...
    // [15]throughput
    // [16]BDP estimate
    // [17]capacity sampled from an ACK mini-window
...
TcpTimeStepEnv::GetObservation()
{
    ...
    // Throughput is normalized by the actual elapsed SP.  In adaptive mode,
    // using a configured interval here can bias samples around RTT changes.
    Time observationInterval = m_timeStep;
    if (m_lastObservationTime.IsStrictlyPositive())
    {
        observationInterval = Simulator::Now() - m_lastObservationTime;
    }
    observationInterval = std::max(observationInterval, MicroSeconds(1));

    // throughput in bit/s
    uint64_t throughput = static_cast<uint64_t>(
        8.0 * segmentsAckedSum * m_tcb->m_segmentSize / observationInterval.GetSeconds());
    box->AddValue(throughput);

    uint64_t trueCapacityBps = g_creoEvaluationCapacityBps;
    uint64_t capacityBps = throughput;
    if (!m_capacitySamples.empty())
    {
        // Delayed ACKs can stretch individual ACK-dispersion samples.  The
        // fastest complete mini-window retains the bottleneck service rate;
        // DWT removes residual short-term compression noise later.
        capacityBps = *std::max_element(m_capacitySamples.begin(),
                                        m_capacitySamples.end());
...
void
TcpTimeStepEnv::TxPktTrace(Ptr<const Packet> packet,
                           const TcpHeader&,
                           Ptr<const TcpSocketBase>)
{
    ...
    if (!m_controlTcb || m_new_pacing_rate == 0 || packet->GetSize() == 0)
    {
        return;
    }

    if (m_burstGapPending)
    {
        // The first packet after the long pacing gap starts a new mini-window.
        m_burstGapPending = false;
        m_burstTxCount = 1;
        m_controlTcb->m_pacingRate = m_controlTcb->m_maxPacingRate;
        return;
    }

    ++m_burstTxCount;
    if (m_burstTxCount >= m_miniWindowSegments)
    {
        // One slow-paced packet interval implements
        // T_pacing = Nm * packet_size / p_send between line-rate bursts.
        uint64_t gapRate = std::max<uint64_t>(1, m_new_pacing_rate / m_miniWindowSegments);
        m_controlTcb->m_pacingRate = DataRate(gapRate);
        m_burstTxCount = 0;
        m_burstGapPending = true;
    }
}
...
TcpTimeStepEnv::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
    ...
    if (m_burstGapPending && m_new_pacing_rate > 0)
    {
        uint64_t gapRate =
            std::max<uint64_t>(1, m_new_pacing_rate / m_miniWindowSegments);
        tcb->m_pacingRate = DataRate(gapRate);
    }
    else
    {
...
TcpTimeStepEnv::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time& rtt)
{
    ...
    m_totalAckedBytes += static_cast<uint64_t>(segmentsAcked) * tcb->m_segmentSize;
    Time now = Simulator::Now();
    if (m_capacityWindowActive && m_lastCapacityAckTime.IsStrictlyPositive() &&
        m_new_pacing_rate > 0)
    {
        Time expectedGap = Seconds(
            8.0 * m_miniWindowSegments * tcb->m_segmentSize / m_new_pacing_rate);
        Time boundaryThreshold = std::max(expectedGap / 2, MilliSeconds(1));
        if (now - m_lastCapacityAckTime > boundaryThreshold)
        {
            // Discard an incomplete window spanning two pacing bursts.
            m_capacityWindowActive = false;
        }
    }
    m_lastCapacityAckTime = now;

    if (!m_capacityWindowActive)
    {
        m_capacityWindowActive = true;
        m_miniWindowStart = now;
        m_capacityWindowStartBytes = m_totalAckedBytes;
        return;
    }

    uint64_t dispersedBytes = m_totalAckedBytes - m_capacityWindowStartBytes;
    uint64_t requiredBytes =
        static_cast<uint64_t>(m_miniWindowSegments - 1) * tcb->m_segmentSize;
    if (dispersedBytes >= requiredBytes)
    {
        Time dispersion = now - m_miniWindowStart;
        if (dispersion.IsStrictlyPositive())
        {
            double sample = 8.0 * dispersedBytes / dispersion.GetSeconds();
